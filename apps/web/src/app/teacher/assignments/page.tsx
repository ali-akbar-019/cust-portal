...
export default function TeacherAssignmentsPage() {
  ...
  useEffect(() => {
    ...
    setSelectedId(null);
    setDetail(null);
    // Split assignments into past and open for tab filtering
    const now = Date.now();
    const pastAssignments = assignments.filter((a) => new Date(a.deadline).getTime() < now);
    const openAssignments = assignments.filter((a) => new Date(a.deadline).getTime() >= now);
    setPast(pastAssignments);
    setOpen(openAssignments);
  }, [sectionId, accessToken, assignments]);
  ...
  return (
    <RoleLayout title="Assignments" items={[]} notificationsHref="#">
        ...
        <div className="mb-4 rounded-md border border-slate-300 overflow-hidden">
          <div className="flex border-b border-slate-200">
            <button
              ...
              onClick={() => setTab('past')}
            >
              Past Due {past.length > 0 ? <span className="ml-1 text-xs text-red-600">({past.length})</span> : ''}
            </button>
            <button
              tabIndex={1}
              role="button"
          ...
          <div className="ledger-card p-6">
            ...
            {assignments.length === 0 ? (
              ...
              <div className="space-y-2">
                {activeTab === 'past' ? past : open}.map((a) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <Ribbon tone={isPast ? 'crimson' : 'muted'>{isPast ? 'closed' : 'open'}</Ribbon>
                    </button>
                  );
                })}
              </div>
