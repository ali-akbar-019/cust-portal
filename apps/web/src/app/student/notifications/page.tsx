...
export default function StudentNotificationsPage() {
  ...
  if (error) return <main className="p-6 lg:p-10"><PageHeader title="Announcements" subtitle="Official notices from the administration and faculty" /><p className="text-sm text-red-600">{error}</p></main>;
  ...
  return (
    <main className="p-6 lg:p-10">
      <PageHeader
        ...
        title="Announcements"
        subtitle="Official notices broadcast to all students, your department, or the courses you are enrolled in."
      />
          ...
          {items.map((a) => (
            <article key={a.id} className="ledger-card p-5">
              <div className="mb-1 flex flex-wrap items-center justify-between gap-2">
                ...
                <Ribbon tone={a.postedBy.role === 'ADMIN' ? 'navy' : 'sapphire'}>
                  {a.postedBy.role}
                </Ribbon>
              </div>
              <p className="mb-3 whitespace-pre-line text-sm text-slate-600">{a.message}</p>
              <p className="text-xs text-slate-400">
                Posted {new Date(a.createdAt).toLocaleString()} by {a.postedBy.email}
              </p>
            </article>
          ))}
        </div>
